...
func _process(delta):
	...
	if Input.is_action_just_pressed("ui_accept") and GameGlobalSingleton.color_game_running and not controls_locked:
		GameGlobalSingleton.player_current_frame = cards.frame
...
func _physics_process(delta):
	...
	if Input.is_action_pressed("ui_right") and not GameGlobalSingleton.color_game_running and not controls_locked:
		velocity.x += SPEED
...
#	# Handle Jump.
	if is_on_floor():
		...
		if Input.is_action_just_released("ui_up") and velocity.y < JUMP_RELEASE_FORCE and not GameGlobalSingleton.color_game_running and not controls_locked:
...
func is_near_villager():
	return near_villager
